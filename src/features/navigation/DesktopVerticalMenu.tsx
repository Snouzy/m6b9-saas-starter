...
const useCurrentPath = (links: NavigationLinkGroups[]) => {
  ...
  const linkMatchCounts = allDashboardLinks.map((link) => ({
    url: link.url,
    matchCount: link.url.split("/").filter((segment, index) => segment === pathSegments[index]).length,
  }));

  const mostMatchingLink = linkMatchCounts.reduce(
    (maxMatchLink, currentLink) => (currentLink.matchCount > maxMatchLink.matchCount ? currentLink : maxMatchLink),
    { url: "", matchCount: 0 },
  );
